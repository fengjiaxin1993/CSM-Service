"""
====================================================================
预警单信息提取 - V2 版本（嵌套输出，支持表格提取）
====================================================================

版本定位：适用于含漏洞表格的复杂预警单，需要提取每个漏洞的详细信息。

核心差异（与 V1 对比）：
  - 使用 try/finally 管理 doc 生命周期
  - 记录倾斜文字作为水印（water_text_set），在表格数据中去除
  - _extract_risk_table()：定位"风险描述"到"影响范围"之间的表格
    · _get_keyword_bbox() 定位关键词坐标
    · find_tables() 查找 PDF 表格
    · _merge_cross_page_tables() 合并跨页表格
    · convert_table() 将表格转为字典列表
  - 输出以 risk_table 为核心，每个漏洞一行
...
class WarningHelper:
    ...
    def __init__(self, path: str):
        ...
        self.span_list = []
        self.risk_table_data = []       # [新增] 表格提取结果
        self.water_text_set = set()     # [新增] 水印文字集合
        try:
                    ...
                    if type_int == 0:  # 只保留文本信息
                        ...
                        for line in lines:
                            ...
                            if direction[0] != 1.0 or direction[1] != 0.0:  # 倾斜文字 → 水印
                                for span in spans:                          # [新增] 记录水印
                                    text = span['text']
                                    self.water_text_set.add(text)
                                continue
                            self.span_list.extend(spans)
            self.text = self._get_text()
            self.water_mark_flag = len(self.water_text_set) > 0             # [新增]
            self._extract_risk_table()                                      # [新增] 表格提取
        finally:
            self.doc.close()
            self.doc = None
    ...
    def extract_info(self) -> dict:
        ...
        与 V1 的核心差异：
        - 不返回顶层的 desc/check/repair/requirement/cve_code
        - 返回 risk_table 列表，每个元素是一个漏洞的完整信息
        - 无表格时退化为单条记录
        """
        dic = {}
